...
{
    /// <summary>
    /// A converter to convert a stream into an ImageSource for Xamarin Forms
    /// </summary>
    public class StreamToImageSourceConverter : IValueConverter
    {
        ...
        {
            if(!typeof(Stream).IsAssignableFrom(targetType)) {
                throw new NotSupportedException();
            }

            var source = value as StreamImageSource;
            if(source == null) {
                return null;
            }

            return source.Stream?.Invoke(CancellationToken.None);
        }
    }
}
