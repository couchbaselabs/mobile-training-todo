...
couchbase_cli_check bucket-create -c 127.0.0.1 \
  -u Administrator -p password --bucket todo \
...
couchbase_cli_check collection-manage -c 127.0.0.1 \
 -u Administrator -p password --bucket todo \
 --create-collection _default.lists

couchbase_cli_check collection-manage -c 127.0.0.1 \
  -u Administrator -p password --bucket todo \
  --create-collection _default.tasks

couchbase_cli_check collection-manage -c 127.0.0.1 \
  -u Administrator -p password --bucket todo \
  --create-collection _default.users

echo "Creating RBAC 'admin' user on todo bucket"
couchbase_cli_check user-manage --set \
  --rbac-username admin --rbac-password password \
  --roles 'bucket_full_access[todo],bucket_admin[todo]' --auth-domain local \
